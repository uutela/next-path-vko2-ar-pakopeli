...
### AC12: The centre is the player, when the player is known
**Given** `points = [POINT_A, POINT_B]` and `player = { latitude: 60.4, longitude: 25.1 }`
**When** `initialCentre(points, player)` is called
**Then** it returns `[25.1, 60.4]` — the player's own position, not any point

A map opens where the player is standing. Centring on a point instead is the
defect this criterion used to contain: the seed's first point decided the view,
so someone playing a route of their own saw a map of Helsinki with their own
point tens of kilometres off screen, and the only way to move the view was to
rename a point so that it sorted first. Reported from a real session.

`src/config/map.ts` has said since week 1 that "a map centred anywhere else
shows the player somewhere they are not". The comment was right and the
function did the opposite of it.

### AC17: Without a position the centre is the first point
**Given** the same points and `player` undefined
**When** `initialCentre(points, undefined)` is called
**Then** it returns `[POINT_A.coordinates.longitude, POINT_A.coordinates.latitude]`

The position arrives asynchronously and may never arrive at all — a refused
permission, a device with no fix. The first point is the fallback, and the
named constant is the fallback after that.

### AC13: With no points and no position the centre falls back to a named constant
**Given** `points = []` and `player` undefined
**When** `initialCentre(points, undefined)` is called
**Then** it returns `MAP_FALLBACK_CENTRE`

### AC18: The map moves to the player once, when the position first arrives
**Given** the map rendered with points and no position
**When** a position arrives
**Then** the map is re-mounted exactly once, with `initialCentre` now returning the player's position

Both platforms configure the view through an *initial* state — MapLibre's
`Camera` on native, `initialViewState` on web — read once when the map mounts
and never again. A position that arrives after mount therefore cannot move a
map that is already open, so the map is keyed on whether a position is known:
`undefined` to known flips once, which re-mounts once. Later movement does not
re-centre, which is deliberate — a map that jumps back every few metres cannot
be panned.
...
| `initialCentre` | happy path | two points and a player position | called | the player's position (AC12) |
| `initialCentre` | edge case | two points, no position | called | the first point's coordinates (AC17) |
| `Map` | edge case | position arrives after mount | re-rendered | the map key changes exactly once (AC18) |
