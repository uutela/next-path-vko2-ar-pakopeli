...
| 20 | DONE | **A stationary player is never stranded.** Proximity is recomputed only from a `LOCATION_CHANGED`, and a device that is not moving sends no more of them — so closing the puzzle returned the screen to `MAP` and left the player standing on an answer point that offered nothing. The shell re-dispatches the last known position on `CLOSE_PUZZLE` and `RESET`.<br>**Done when:** a named test collects and closes a puzzle with exactly one position delivered and asserts `Syötä koodi` appears without a second one; `npm test` and `npx tsc --noEmit` exit 0; the browser run stays green. |
| 21 | DONE | **The puzzles come from an agent.** `agents/puzzle-agent` writes Finnish brain teasers with Gemini and proves each one before handing it over — schema, solve-back against a second call that sees only the text, and a duplicate check against memory. Three attempts, then a refusal with a reason. Reached over HTTP behind the existing `PuzzleSource`; a refusal falls back to the local generator and is reported, never swallowed.<br>**Done when:** ten consecutive puzzles pass every check against a fake model, `pytest` passes offline with no API key and opens no socket, `npm test` and `npx tsc --noEmit` exit 0, and the browser run shows the absent agent still producing a puzzle. |
