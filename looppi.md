...
| 7 | TODO | **Drawing moves out of the pure function.** `transition` stops calling the generator and receives a drawn puzzle as an event instead; it stays pure — no clock, no network, no GPS.<br>**Done when:** `grep -rn "generatePuzzle" src \| cut -d: -f1 \| sort -u` no longer lists `src/domain/gameState.ts`, `transition` has no `await` and no import of the source, a named test asserts the same event twice does not redraw, `npm test` exits 0. |
